...
public class FlinkSinkBuilder {
    ...
    public FlinkSinkBuilder clusteringIfPossible(
            ...
            int sampleFactor) {
        // 1. The clustering will be skipped if the clustering columns are empty or the execution
        // mode is STREAMING.
        ...
        if (!boundedInput) {
            LOG.warn(
                    "Clustering is enabled; however, it has been bypassed as "
                            + "it only supports the BATCH execution mode.");
            return this;
        }
        // 2. Check the table type.
        checkState(
                table.bucketMode().equals(BUCKET_UNAWARE),
                "Clustering only supports bucket unaware table without primary keys.");
        // 3. Check the clustering columns.
        ...
        checkState(
                ...
                String.format(
                        ...
                        fieldNames, columns));
        // 5. Check the sample factor.
        int minSampleFactor = 20;
        checkState(
                sampleFactor >= minSampleFactor,
                "The minimum allowed "
        ...
        int localSampleSize =
                upstreamParallelism > 0
                        ...
                        : sinkParallelism * minSampleFactor;
        this.tableSortInfo =
                sortInfoBuilder
